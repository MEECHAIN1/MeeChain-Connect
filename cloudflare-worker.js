...
// ══════════════════════════════════════════════════════════════════
// CONFIG — แก้ตรงนี้ให้ตรงกับ origin server ที่ใช้งาน
// ══════════════════════════════════════════════════════════════════
const CONFIG = {
  // ─── Option A: Replit Deployed App (แนะนำ ✨) ───────────────────
  // ตั้งค่า ORIGIN_URL เป็น URL ของ Replit deployed app เช่น:
  //   'https://mguzkmkp.pike.replit.app'
  // จากนั้นตั้ง ORIGIN_ONLINE: true เพื่อให้ Worker proxy มาที่นี่
  ORIGIN_URL:    'https://mguzkmkp.pike.replit.app',

  // ─── Option B: Home/VPS Server (IP:Port) ────────────────────────
  // ใช้เมื่อรัน server ที่บ้าน — ต้องเปิด port forward ก่อน
  ORIGIN_IP:     '58.11.89.11',
  ORIGIN_PORT:   5000,   // เปลี่ยนจาก 8080 → 5000 (server.js ใหม่)

  // ─── เลือก mode ─────────────────────────────────────────────────
  // 'replit'  = proxy ไป ORIGIN_URL (Replit app) ← แนะนำ
  // 'homelab' = proxy ไป ORIGIN_IP:ORIGIN_PORT (home server)
  // 'mock'    = ตอบ mock เท่านั้น (ไม่ proxy)
  ORIGIN_MODE: 'mock',   // ← เปลี่ยนเป็น 'replit' หลัง deploy แล้ว
  ...
  ALLOWED_ORIGINS: [
    ...
    'https://app.meechain.run.place',
    'https://mguzkmkp.pike.replit.app',
    'https://mguzkmkp.pike.replit.dev',
    'http://localhost:5000',
    'http://localhost:3000',
    'http://localhost:8080',
  ],
...
// helper — ต้องไม่เปลี่ยน
const ORIGIN_ONLINE = CONFIG.ORIGIN_MODE !== 'mock';
function getOriginBase() {
  if (CONFIG.ORIGIN_MODE === 'replit') return CONFIG.ORIGIN_URL.replace(/\/$/, '');
  if (CONFIG.ORIGIN_MODE === 'homelab') return `http://${CONFIG.ORIGIN_IP}:${CONFIG.ORIGIN_PORT}`;
  return null;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === 'GET' && (path === '/health' || path === '/api/health')) {
      return jsonResponse({
        status:  'ok',
        worker:  'MeeChain-CF-Worker-v2.1',
        chainId: CONFIG.CHAIN_ID,
        chainIdHex: CONFIG.CHAIN_ID_HEX,
        chainName: 'MeeChain Ritual Chain',
        rpc:     `https://rpc.meechain.live`,
        rpcMode: CONFIG.ORIGIN_MODE,
        origin:  getOriginBase() || 'mock-only',
        originOnline: ORIGIN_ONLINE,
    ...
    if (method === 'POST' && (path === '/' || path === '/rpc' || path === '/api/rpc')) {
      ...
      if (ORIGIN_ONLINE) {
        try {
          const originUrl = `${getOriginBase()}/rpc`;
    ...
    if (ORIGIN_ONLINE) {
      try {
        ...
        const originUrl  = `${originBase}${path}${url.search}`;
        ...
        resHeaders.set('X-Served-By', 'MeeChain-CF-Worker-v2.1');
        ...
        return jsonResponse({
          ...
          message: `Origin server offline (mode: ${CONFIG.ORIGIN_MODE})`,
          detail:  proxyErr.message,
          fixes: [
            'mode=replit: ตรวจสอบว่า Replit app deployed แล้วและ URL ถูกต้องใน ORIGIN_URL',
            'mode=homelab: ตรวจสอบ Router port forward และ Firewall ที่ origin server',
            `ทดสอบ: curl ${getOriginBase()}/health`,
          ],
          worker: 'MeeChain-CF-Worker-v2.1',
          ts: new Date().toISOString(),
        }, 502, cors);
